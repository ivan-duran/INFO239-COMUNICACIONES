...
// Simulación de cifrado asimétrico (clave pública)
const uint8_t CLAVE_PUBLICA = 77;

uint8_t cifrarSimulado(uint8_t byte) {
...
void loop() {
  ...
  while (true) {
    for (int i = 0; i < TOTAL_PAQUETES; i++) {
      ...
      paquete[3] = cifrarSimulado(imagenYinYang[i][0]);
      paquete[4] = cifrarSimulado(imagenYinYang[i][1]);
      paquete[5] = cifrarSimulado(imagenYinYang[i][2]);
